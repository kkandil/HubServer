...
function createMultiGateway({store,appToken,bindings}) {
  ...
  async function statuses() { const result=await store.read('GetAllHomes',{},online); io.emit('HomeStatuses',result); }
  ...
  io.use((s,next)=>next(same(s.handshake.query.token,appToken)?undefined:new Error('Unauthorized')));
  const ns=io.of('/hub');
  ...
  ns.on('connection',s=>{
    ...
    s.on('HomeRuntime',safe(async data=>{if(hubs.get(name)!==hub || data.homeName!==name) return; await store.repo.runtime(name,data); await statuses();io.emit('EventsChanged',{homeName:name});}));
    s.on('PhoneResponse',data=>{
      ...
      const phone=phones.get(data.id); if(phone) phone.emit(data.event,data.payload);
    });
  ...
  io.on('connection',s=>{
    ...
    safe(async()=>s.emit('HomeStatuses',await store.read('GetAllHomes',{},online)))();
