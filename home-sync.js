  ...
  async apply(doc) {
    ...
    return this.events.serial(async()=>{
      ...
      try {
        ...
        const oldVariables=new Map(previous.map(d=>[d.id,sql.prepare('SELECT body FROM documents WHERE db=? AND collection=?').all(this.homeName,'Var_'+d.Name).map(r=>JSON.parse(r.body))]));
        ...
        for(const device of doc.devices) {
          ...
          const variables=oldVariables.get(device.id)||[];
        ...
        for(const old of previous) if(!doc.devices.some(d=>d.Name===old.Name)) {
          sql.prepare('DELETE FROM documents WHERE db=? AND collection=?').run(this.homeName,'Var_'+old.Name);
          sql.prepare('DELETE FROM collections WHERE db=? AND name=?').run(this.homeName,'Var_'+old.Name);
        }
      ...
      for(const connection of this.connections.values()) {
        const device=doc.devices.find(d=>d.id===connection.DeviceId);
        if(connection.HomeName===this.homeName && device)connection.Name=device.Name;
      }
